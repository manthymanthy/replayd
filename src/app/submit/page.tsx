...
// Accettiamo: YouTube, Twitch Clips, Twitch VOD
const SAFE_URL =
  /^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be|clips\.twitch\.tv|www\.twitch\.tv\/clips|twitch\.tv\/videos\/\d+)/i;
...
export default function SubmitPage() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!SAFE_URL.test(u)) {
      setMsg({ type: 'err', text: 'For now we only accept YouTube, Twitch Clips, or Twitch VOD links.' });
      return;
    }
  ...
  return (
    <section className="page">
      ...
      <form onSubmit={onSubmit} className="stack" noValidate>
        <div className="fieldWrap">
          ...
          <input
            ...
            placeholder="YouTube / Twitch clip or Twitch VOD URL"
            className="field"
            required
            inputMode="url"
            aria-invalid={!url}
      ...
      <style
        ...
        .btn{
          padding:12px 16px; border-radius:10px; border:1px solid var(--line-strong);
          background:color-mix(in oklab, var(--panel) 88%, #fff 12%);
          color:#fff; font-weight:800; letter-spacing:.02em; cursor:pointer;
